...
const ToolCard = ({ logo, name, description, tags, bookmarks, votes, redirectUrl, gradient, isPopular }) => {
  ...
  return (
    <div 
      className={`group relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl  shadow-2xl p-8 mb-6 border border-gray-700/50 hover:border-purple-500/50 transition-all duration-500 cursor-pointer transform hover:scale-[1.02] hover:shadow-purple-500/20 ${isHovered ? 'shadow-2xl' : ''}`}
    ...
    >
      ...
      {/* Hover glow effect */}
      <div className="absolute inset-0  bg-gradient-to-r from-purple-600/10 via-blue-600/10 to-pink-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
    </div>
  );
};
...
const AppUI = () => {
  ...
  const categories = [
    { icon: <MessageSquare />, label: 'AI Chatbots & Assistants' },
    { icon: <Palette />, label: 'Design & Creative Tools'},
    { icon: <Wrench />, label: 'Writing & Content' },
    { icon: <Briefcase />, label: 'Business & Productivity' },
    { icon: <TrendingUp />, label: 'Marketing & Growth' },
    { icon: <Lightbulb />, label: 'Research & Analysis' },
    { icon: <Zap />, label: 'Developer Tools' },
